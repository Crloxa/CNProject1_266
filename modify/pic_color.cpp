...
namespace ImgParse {
    ...
    void drawDataGridAndStars(Mat& img) {
        double k = img.cols / 133.0;
        int radius = std::max(1, cvRound(0.1 * k));
            ...
            for (int c = 0; c < 133; ++c) {
                if (isDataOrHeaderCell(r, c)) {
                    ...
                    if (cx >= 0 && cx < img.cols && cy >= 0 && cy < img.rows) {
                        ...
                        circle(img, Point(cx, cy), radius, stdColor, -1);
                    }
                }
            }
        }
    ...
    void drawPaletteMarkers(Mat& img) {
        double k = img.cols / 133.0;
        int radius = std::max(2, cvRound(0.3 * k));

        for (int i = 0; i < 8; ++i) {
            int cx = cvRound((21.5 + i) * k);
            int cy = cvRound(6.5 * k);
            circle(img, Point(cx, cy), radius, stdColors[i], -1);
        }
    }
    ...
    bool processV5(const Mat& srcImg, Mat& disImg) {
        ...
        vector<Point2f> reorderedSrc(4);
        for (int i = 0; i < 4; ++i) {
            reorderedSrc[i] = srcPointsOuter[(smallQrIdx + 2 + i) % 4];
        }

        double est_L = std::min(norm(reorderedSrc[1] - reorderedSrc[0]), norm(reorderedSrc[3] - reorderedSrc[0]));
        int targetSize = cvRound(est_L);

        vector<Point2f> dstTarget = {
            Point2f(0.0f, 0.0f), Point2f(targetSize, 0.0f),
            Point2f(targetSize, targetSize), Point2f(0.0f, targetSize)
        };

        lastValidTransform = getPerspectiveTransform(reorderedSrc, dstTarget);
        lastValidTargetSize = targetSize;

        warpPerspective(srcImg, disImg, lastValidTransform, Size(targetSize, targetSize), INTER_CUBIC);
    ...
    bool processV15(const Mat& srcImg, Mat& gray, Mat& disImg, bool useHSV) {
        ...
        double est_L = std::min(norm(TR - TL), norm(BL - TL)) / 112.0 * 133.0;
        int targetSize = cvRound(est_L);
        double k_warp = targetSize / 266.0;

        vector<Point2f> srcPoints = { TL, TR, BR, BL };
        vector<Point2f> dstPoints = {
            Point2f(21.0f * k_warp, 21.0f * k_warp),
            Point2f(245.0f * k_warp, 21.0f * k_warp),
            Point2f(253.33f * k_warp, 253.33f * k_warp),
            Point2f(21.0f * k_warp, 245.0f * k_warp)
        };
        ...
        warpPerspective(srcImg, disImg, transformMatrix, Size(targetSize, targetSize), INTER_CUBIC);
    ...
    bool Main(const cv::Mat& srcImg, cv::Mat& disImg) {
        ...
        if (!lastValidTransform.empty()) {
            warpPerspective(srcImg, disImg, lastValidTransform, Size(lastValidTargetSize, lastValidTargetSize), INTER_CUBIC);
